<TITLE>PENDING TAHAP TOPPING OFF</TITLE>
<style type="text/css">
<!--
.style1 {
...
<form method=get action=cari_progress_cair_2.php>
  ...
  <p class="style11">  <INPUT type=radio name=pilih value=tgl_cair_2  checked> 
  Tahap Topping Off <br>
  <br>
...
if($pilih == "tgl_cair_1")
{
$a = "DAFTAR DEBITUR IN PROGRESS TAHAP PONDASI";
}
...
$tampil=mysql_query("SELECT * FROM data WHERE data.lnc LIKE '$lnc' AND data.tgl_cair_1 >'0000-00-00' AND $pilih ='0000-00-00' AND data.progress='BELUM'    
                    ORDER BY data.tgl_pk ASC");
$jumlah= mysql_num_rows($tampil);

//ngitung jumlah 
$max    = "SELECT SUM(max_kredit) AS total_max FROM data WHERE data.progress='BELUM' AND data.lnc LIKE '$lnc' AND $pilih='0000-00-00' AND data.tgl_cair_1 > '0000-00-00'";
$result = mysql_query($max) or die 
         (mysql_error());
  $t    = mysql_fetch_array($result);
$xxx    = $t['total_max'];

$cair1  = "SELECT SUM(cair_1) AS total_max FROM data WHERE data.progress='BELUM' AND data.lnc LIKE '$lnc' AND $pilih='0000-00-00' AND data.tgl_cair_1 > '0000-00-00'";
$result = mysql_query($cair1) or die 
         (mysql_error());
  $t    = mysql_fetch_array($result);
$xxx1   = $t['total_max'];

$cair2  = "SELECT SUM(cair_2) AS total_max FROM data WHERE data.progress='BELUM' AND data.lnc LIKE '$lnc' AND $pilih='0000-00-00' AND data.tgl_cair_1 > '0000-00-00' ";
$result = mysql_query($cair2) or die 
         (mysql_error());
  $t    = mysql_fetch_array($result);
$xxx2   = $t['total_max'];

$cair3  = "SELECT SUM(cair_3) AS total_max FROM data WHERE data.progress='BELUM' AND data.lnc LIKE '$lnc' AND $pilih='0000-00-00' AND data.tgl_cair_1 > '0000-00-00' ";
$result = mysql_query($cair3) or die 
         (mysql_error());
  $t    = mysql_fetch_array($result);
$xxx3   = $t['total_max'];

$cair4  = "SELECT SUM(cair_4) AS total_max FROM data WHERE data.progress='BELUM' AND data.lnc LIKE '$lnc' AND $pilih='0000-00-00' AND data.tgl_cair_1 > '0000-00-00' ";
...
$tampil2    = mysql_query("SELECT * FROM data WHERE data.progress='BELUM' AND data.lnc LIKE '$lnc' AND $pilih='0000-00-00' AND data.tgl_cair_1 > '0000-00-00'");
