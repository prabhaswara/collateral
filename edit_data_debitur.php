...
include 'collateral_script/control_frmedit.php';
...
?>
                ...
                /*
                 $('#searchNoApp').click(function() {
                 $.ajax({ // ajax call starts
                 url: 'col_json.php?mod=json_nomor_aplikasi&noaplikasi='+$('#noaplikasi').val(),                    
                 dataType: 'json',
                 ...
                 $.each(data.listProgram, function(k, v) {
                 opt+="<option value='"+v+"' >"+v+"</option>";
                 });
                 $('#program').html(opt);
                 
                 $('#produk').val(data.field.produk);
                 $('#program').val(data.field.program);
                 $('#inputdate').val(data.field.inputdate);
                 $('#lnc').val(data.field.lnc);
                ...
                $('#produk').change(function() {
                    ...
                    $.ajax({// ajax call starts
                        url: 'col_json.php?mod=json_list_program&produk=' + $('#produk').val(),
                        dataType: 'json',
                        success: function(data)
                        {
        ...
        <form method="POST">
            <div style="margin:0px 50px;text-align: left;">
                ...
                <table class="tbllayout">
                    <tr>
                        <td>
                            <a style='float:left;width:100px' href="#"  class="classbtn" onclick="javascript:window.history.back(-1);">Kembali</a>
                            <a style='float:left;width:100px' href="col_trail.php?id=<?= $_GET['id'] ?>" class="classbtn">Lihat Trail</a>
                        ...
                        <tr>
                            <td>
                                <table class="tbllayout" >
                                    ...
                                    <tr>
                                        <td class="w180">LNC</td><td class="w300"><?= ht_input("lnc") ?></td>
                                    </tr>
                                    <tr>
                                        <td class='tambahan'>Input Date</td><td>

    <?= ht_input("input_date", "class='dateNormal dateMask'") ?>
    ...
    <?= ht_select("program", $ListProgram); ?>
                            ...
                            <td>
                                <table class="tbllayout" >                                    
                                    ...
                                    <tr>                          
                                        <td>Norek Pinjaman <span class="red">*)</span></td><td> 
    <?= ht_input("no_rekg_pinjaman", "", "hidden") ?>
    <?= cleanstr($_POST['frm']['no_rekg_pinjaman']) ?>
